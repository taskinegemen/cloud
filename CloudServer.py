...
def DeleteFromCatalog(channel,details):
    print "DeleteFromCatalog komutu alindi"
    catalog_ips = ConfigSectionMap("Server")['catalog_ips']
    print "catalog ips:"+catalogip
    incoming_ip=details[0]
    print "incoming ip:"+incoming_ip
    DeployFolder = ConfigSectionMap("Server")['deployfolder']
    if incoming_ip in catalog_ips :
        print "DeleteFromCatalog komutu alindi!"
        channel.send ('DeleteFromCatalogStarted')
...
# Set up the server:
catalogip = ConfigSectionMap("Server")['catalog_ips']
print catalogip
